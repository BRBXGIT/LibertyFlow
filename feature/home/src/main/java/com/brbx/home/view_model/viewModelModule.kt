...
internal val viewModelModule = module {
    viewModel {
        ...
        val authProcessor = getCommonAuthSheetProcessor(
            lens = HomeState.authSheetState
        )

        val selectionProcessor = getCommonSelectionProcessor(
            lens = HomeState.selection,
            onUnauthorized = {
                with(receiver = authProcessor) {
                    process(intent = CommonAuthSheetIntent.ToggleSheet)
                }
            }
        )

        val searchProcessor = getCommonSearchProcessor(
            lens = HomeState.search
        )
